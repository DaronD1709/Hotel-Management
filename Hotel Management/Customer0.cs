...
namespace Hotel_Management
{
    internal class Customer0
    {
        ...
        public bool insertCustomer(int idcustomer, string namecustomer, string idcard, int gender, DateTime dobcustomer, string phonenumber, string national)
        {
        ...
        public bool UpdateCustomer(int idcustomer, string namecustomer, string idcard, int gender, DateTime dobcustomer, string phonenumber, string national)
        {
            // Câu lệnh SQL để cập nhật thông tin khách hàng
            SqlCommand command = new SqlCommand("UPDATE std SET name = @name, Idcard = @Idcard, gender = @gdr, dob = @dob, phone = @phone, nation = @nation WHERE id = @id", mydb.getConnection);
